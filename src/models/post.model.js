...
module.exports = class PostModel {
    ...
    // create post []
    create = (entity) => db.insert(TBL_POSTS, entity);

    update = (entity, user_id, post_id) => db.update2condition(TBL_POSTS, entity, {user_id: user_id}, {id: post_id});

    findOne = async function (user_id, post_id) {
        const rows = await db.load(`SELECT *
                                    FROM ${TBL_POSTS}
                                    WHERE ${TBL_COL_USER_ID} = '${user_id}'
                                      AND ${TBL_COL_ID} = '${post_id}'`);
        if (rows.length === 0) {
            return null
        }
        return rows[0];
...
// PostModel.delete = (user_id, post_id) => db.delete2condition(TBL_POSTS, { user_id: user_id }, { id: post_id })
    ...
    delete = (entity, user_id, post_id) => db.delete2condition(TBL_POSTS, entity, {user_id: user_id}, {id: post_id})
